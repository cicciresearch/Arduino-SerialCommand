...
SerialCommand::SerialCommand(Stream& port,           //
  ...
  _checkAddressMatch = checkAddressMatch; // Set the address matching flag
...
void SerialCommand::setDeviceAddress(int address) {
  // Set the device address for command filtering
  _device_address = address;
#ifdef SERIALCOMMAND_DEBUG
  DEBUG_PORT.print("Device address set to: ");
  DEBUG_PORT.println(_device_address);
#endif
}
...
void SerialCommand::processCommand() {
  ...
  if (_checkAddressMatch) {
    char* addressStr = strtok_r(NULL, _delim, &_last); // e.g. "2"
    int receivedAddress = atoi(addressStr);

    addressMatch = false;
    if (_device_address <= 0 || receivedAddress == 0) {
      addressMatch = true; // Accept addresses 0 and -1 regardless
    } else if (addressStr) {
